...
@Component({
    ...
    <div class="booking-container py-5">
      ...
      <main class="content px-4">
        ...
        <div class="map-snippet mb-4">
          <iframe 
            src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3559.6105658602283!2d80.9424687!3d26.8524458!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2zMjbCsDUxJzA4LjgiTiA4MMKwNTYnMzIuOSJF!5e0!3m2!1sen!2sin!4v1711995600000!5m2!1sen!2sin" 
            width="100%" height="160" style="border:0; border-radius: 20px;" allowfullscreen="" loading="lazy">
          </iframe>
        </div>

        <h2 class="section-title mb-4">Booking {{ parking().name }}</h2>
        ...
        <div class="details-card p-4 mb-4">
          <div class="d-flex justify-content-between align-items-start">
            <div class="info-group">
              ...
              <h3 class="value">{{ parking().zone }}</h3>
              
              <span class="label mt-3">Selected Slot</span>
        ...
        <div class="duration-section mb-5">
          <h4 class="sub-title mb-3">Time & Price</h4>
          <div class="duration-grid">
            <button 
              *ngFor="let option of durationOptions" 
              class="duration-btn"
              [class.active]="selectedDuration() === option.hours"
              (click)="selectedDuration.set(option.hours)"
            >
              <span class="btn-hours">{{ option.hours }} Hour</span>
              <span class="btn-price">\${{ option.hours * parking().rate }}</span>
            </button>
          </div>
        </div>

        <!-- Start Booking Button -->
        <button (click)="startBooking()" class="btn-start-booking w-100 py-3">
          Start Booking
        </button>
      </main>
    </div>
  ...
  styles: [`
    ...
    .duration-grid::-webkit-scrollbar {
      display: none;
    }
    ...
    .duration-btn.active .btn-hours,
    .duration-btn.active .btn-price {
      color: white;
    }
...
export class ParkingBookingComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.queryParams.subscribe(params => {
      if (params['name']) {
        this.parking.update(p => ({
          ...p,
          name: params['name'],
          address: params['address'] || p.address,
          rate: parseInt(params['rate']) || p.rate,
          type: params['type'] || p.type
        }));
      }
    });
  }
  ...
  startBooking() {
    alert(`Booking successful for ${this.selectedDuration()} hours at ${this.parking().name} starting ${this.bookingDate} ${this.bookingTime}! Total: $${this.totalPrice()}`);
